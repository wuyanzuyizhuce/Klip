...
const detectCategory = (text: string) => {
  const t = text.trim()
  if (/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(t) || /^rgb\(/i.test(t)) return 'color'
  if (/^(https?:\/\/[^\s]+)/g.test(t)) return 'link'
  const codeKeywords = ['const', 'let', 'var', 'function', 'class', 'import', 'export', '=>', '{', '}', 'console.log', 'println!', 'public static', 'def ', 'fn ']
  if (codeKeywords.some(k => t.includes(k)) || (t.length > 20 && t.includes(';'))) return 'code'
  return 'text'
}
...
const HighlightedText = ({ text, highlight }: { text: string, highlight: string }) => {
  ...
  return (
    <span>
      {parts.map((part, i) => 
        regex.test(part) ? (
          <span key={i} className="bg-blue-500/30 text-blue-200 rounded-sm px-0.5 border-b border-blue-400/50">{part}</span>
        ) : (
...
function App() {
  ...
  }, [currentItem, selectedIds, history, clearSelection, filteredHistory.length, selectedIndex, prevItem])
  ...
  }, [filteredHistory, selectedIndex, isActionMenuOpen, actionIndex, selectedIds, currentItem, showFullImage, actions, movingToCollection, searchQuery, handleAction, nextItem, prevItem, toggleSelection])
  ...
  return (
    <motion.div 
      ...
      className="h-screen w-screen flex flex-col rounded-2xl overflow-hidden font-sans bg-stone-900/95 relative text-stone-50 border border-white/5 shadow-2xl drag"
    >
      ...
      <div className="flex-1 flex overflow-hidden min-h-0 relative bg-stone-900/60">
        ...
        <div className="flex-1 min-w-0 flex flex-col relative bg-black/10 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-3 space-y-2 scroll-smooth custom-scrollbar no-drag">
            <motion.div
                      ...
                      <div className="flex-1 min-w-0 pr-10 z-10 flex flex-col justify-center min-h-[36px]">
                        {item.type === 'image' ? (
                          ...
                          <div className="flex flex-col">
                            ...
                            >
                              <HighlightedText text={item.fileName || '未知文件'} highlight={debouncedQuery} />
                            </p>
                          ...
                          >
                            <HighlightedText text={item.content} highlight={debouncedQuery} />
                          </p>
                        )}
